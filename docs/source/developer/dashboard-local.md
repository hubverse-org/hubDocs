...
This page will demonstrate that workflow using a practical example of building [the flu metrocast dashboard](https://reichlab.io/metrocast-dashboard) locally.
The important takeaway from this exercise is that it exists
**independent of GitHub workflows**. We are not concerned
about where data are stored or when they are updated.

## Rationale

Building a dashboard website requires three tools:

1. one to build the static website (<https://github.com/hubverse-org/hub-dash-site-builder>)
1. one to build the predtimechart data and options (<https://github.com/hubverse-org/hub-dashboard-predtimechart>)
1. one to build the predevals data and options (<https://github.com/hubverse-org/hubPredEvalsData-docker>)
...
After this demo, we will end up with a temporary directory that
...
|:-----|:-------|-----------:|
| `dashboard/` | source for the hub dashboard | humans |
...
1. create a temporary directory and save it in `$tmp`; define the dashboard
   repository
   ```bash
   tmp=$(mktemp -d)
...
4. generate the target data. This takes three arguments, the hub, the
   configuration file, and finally the output directory.
   ```bash
...
working directory, so the strategy here is to link the dashboard directory to
the `/site` working directory and reference any files or directories with
