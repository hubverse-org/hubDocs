...
The `hub-config` directory in a modeling hub is required to contain three JSON[^json] files:
   ...
   2. `tasks.json`{.codeitem} - JSON file specifying modeling tasks and model output formats, which may be round-specific. See [the hub model task configuration (`tasks.json` file)](#tasks-metadata) below for more details on the `tasks.json` file.  
