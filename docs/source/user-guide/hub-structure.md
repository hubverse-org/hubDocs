...
Here's an example of an oracle data in the `target-data/oracle-output` directory partitioned by target_end_date:
...
Hubs can use their own file naming convention or can retain the file names that are generated by the library they use for partitioning. Since hive-partitioned datasets are not expected to store data in the file names themselves, tooling to read them ignores file names altogether. 
