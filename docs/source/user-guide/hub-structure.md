...
* Partitioned target data files should be stored in the `target-data` directory, in either a `target-data/time-series` or `target-data/oracle-output` subdirectory.
